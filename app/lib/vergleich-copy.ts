...
const stylink: CopyBuilder = (c, own) => ({
  ...
  handling: [
    ...
    `Der zweite Unterschied ist die Vertragsseite. Bei stylink UGC läuft die Abwicklung laut Anbieterangabe über die Plattform. Damit ist die Plattform der Ansprechpartner für Auftrag und Zahlung. Im Direktmodell bist du selbst die Instanz: Du vereinbarst Liefertermin und Abnahme, du zahlst, und du klärst die Nutzungsrechte. Wie lange darfst du das Video schalten, auf welchen Kanälen, in welchen Märkten? Diese Frage muss beantwortet werden, bevor gedreht wird — auf jeder Plattform und in jedem Direktvertrag.`,
