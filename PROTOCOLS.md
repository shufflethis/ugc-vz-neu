...
| **WebMCP** | W3C-Proposal (webmachinelearning/webmcp, Stand Aug 2026); beide API-Formen: `registerTool` (inkrementell) und `provideContext` (deklarativ), auf `document.modelContext` (ChatGPT Site tools) wie `navigator.modelContext` (Chromium-Prototyp), mit Retry-Polling für spät injizierte APIs; reine Getter tragen `readOnlyHint` | Browser-seitig: `app/components/WebMcpProvider.tsx` (eingebunden im Root-Layout). 7 Tools – Registry-Teilmenge ohne `request_outreach` (Human-in-the-loop: die Kontaktanfrage sendet der Mensch im Formular; danach liefert `get_last_outreach` die request_id). `select_creators` steuert die echte Such-UI. `get_human_selection` liest die Klick-Markierung des Menschen (Rückkanal Mensch → Agent). Sichtbar nur in WebMCP-fähigen Browsern, nicht im HTML. |
