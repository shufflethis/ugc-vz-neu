...
those cards and opens the contact form pre-filled. What used to be a
copy-paste relay between a chat window and a website becomes one shared
...
A client-side provider (`app/components/WebMcpProvider.tsx`) registers 6
...
existing React search component; read-only tools carry
...
is verified by an end-to-end test in a real Chromium against production.
...
with 6 tools. Say: "Find me three beauty creators for TikTok product
...
(submitting sends a real e-mail, so only submit if you mean it). No login
...
2. **0:20–0:40 — Discovery.** Click the "Site tools" entry in the address
   bar, expand "Available site tools". Voice: "The page registers six tools
...
4. **1:30–2:10 — Selection and the human gate.** Type: "Select the first and
   the third." Cards get marked, the contact form opens pre-filled. Voice:
   "select_creators marks them and opens the request form. And here's the
...
5. **2:10–2:40 — Closing the loop.** Type: "What's the status of my
...
6. **2:40–2:55 — Facts.** Voice over repo/README: "Built during the
